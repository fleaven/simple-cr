...
def basiccnn1(input_tensor,train,regularizer,channels):
	conv1_deep = 96
	conv2_deep = 128
	conv3_deep = 160
	conv4_deep = 256
	...
	with tf.name_scope("layer-ss0"): #48*4
		kV = tf.constant(100.)
		bV = tf.constant(2.)
		x_bin, x_total = SumDiff(tf.sigmoid((h_pool1 - bV) * kV), 1)  # 对比度最大化后处理

	with tf.name_scope("layer-ss1"):    #24*4
		w_conv_s1 = weight_variable([5, 1, conv1_deep, convs1_deep])
		b_conv_s1 = bias_variable([convs1_deep])
		x_bin1 = tf.nn.relu(conv2d(x_bin, w_conv_s1) + b_conv_s1)

	with tf.name_scope("layer-sspool1"):
		h_sspool1 = max_pool_2x1(x_bin1) #48

	with tf.name_scope("layer-ss2"):    #12*4
		w_conv_s2 = weight_variable([5, 1, convs1_deep, convs2_deep])
		b_conv_s2 = bias_variable([convs2_deep])
		x_bin2 = tf.nn.relu(conv2d(h_sspool1, w_conv_s2) + b_conv_s2)

	with tf.name_scope("layer-sspool2"):
		h_sspool2 = max_pool_2x1(x_bin2) #48

	with tf.name_scope("layer-ss3"):    #6*4
		w_conv_s3 = weight_variable([5, 1, convs2_deep, convs3_deep])
		b_conv_s3 = bias_variable([convs3_deep])
		x_bin3 = tf.nn.relu(conv2d(h_sspool2, w_conv_s3) + b_conv_s3)

	with tf.name_scope("layer-sspool3"):
		h_sspool3 = max_pool_2x1(x_bin3)  # 48

	pool_shape = h_pool4.get_shape().as_list()
	nodes = pool_shape[1] * pool_shape[2] * pool_shape[3]
	x_shape = h_sspool3.get_shape().as_list()
	x_nodes = x_shape[1] * x_shape[2] * x_shape[3]
	reshaped = tf.concat([tf.reshape(h_pool4, [-1, nodes]), tf.reshape(h_sspool3, [-1, x_nodes]), tf.reshape(x_total, [-1, 4*conv1_deep])], 1)

	with tf.name_scope("layer7-fc1"):
		w_fc1 = weight_variable([nodes + x_nodes+4*conv1_deep, fc1_num])
...
#        keep_prob = tf.placeholder("float")
	...
	return tf.matmul(h_fc1_drop, w_fc2) + b_fc2, h_pool1